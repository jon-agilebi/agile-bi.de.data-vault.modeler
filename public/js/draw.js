...
function saveElements() {
	...
	for(var i = 0; i < elements.length; i++) {
		if(elements[i].reference) {
			if(getConnectedElements(elements[i].frame).length > 0) serialized.push({elementId:elements[i].elementId, x:elements[i].frame.position.x - 90, y:elements[i].frame.position.y - 30, kind:elements[i].kind, text:elements[i].text.content, configuration:elements[i].configuration, reference:elements[i].reference });
		}
